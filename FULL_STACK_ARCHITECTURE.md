...
- eventually support signed local firmware updates with rollback, and
...
Assistant for an RF ID or captured transcript. It passed end to end on Sensor A;
Sensor B repetition and the persistence/interruption matrix remain release
gates. Authenticated node protocol v2 allows `rainpointd` to start or cancel
...
- Automatic factory-identity adoption physically proven on Sensor A
- Home Assistant node adoption, Identify, diagnostics, and sensor pairing UI

Remaining release gates are automatic pairing on Sensor B, restart/rejoin
behavior, unattended reporting, interruption recovery, and reassociation
semantics. These are qualification tasks rather than missing protocol-core
features.
