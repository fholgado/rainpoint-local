...
| In progress | Live receive code previously owned this house's sensor names and singleton valve state | A device-catalog boundary now separates RF decoding from installation identity and supports independent per-valve state while preserving legacy HA IDs | Replace the explicit legacy compatibility catalog with persistent user-managed identities before publication |
...
| `rainpointd_addon/rainpointd/rtl433.py` | Adapter with legacy defaults | Process management is now separate from transport-neutral frame ingestion, but the default compatibility catalog and `seed()` still create this installation's devices before RF is heard | Load identities from the persistent registry and remove compatibility seeding from publishable production mode |
| `rainpointd_addon/rainpointd/ingest.py` | Production candidate | Provides one catalog-aware frame-to-device/event path shared by SDR, serial, and network adapters, including independent per-valve state | Evolve its dictionary boundary into typed protocol observations and registry policy |
