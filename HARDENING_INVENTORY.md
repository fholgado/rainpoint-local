...
| In progress | Forget/delete semantics span SQLite, a separate pairing JSON file, in-memory devices, and HA registries | Gateway forget now atomically removes registry metadata, persists endpoint suppression, removes current state, and prevents RF rediscovery until explicit acceptance; pairing JSON and HA entity/device reconciliation remain separate | Consolidate enrollment state and add HA removal/disable reconciliation around the gateway lifecycle |
...
| Path | Classification | Finding | Disposition |
| --- | --- | --- | --- |
| `rainpointd_addon/rainpointd/storage.py` | Production candidate | The device registry now drives HCS026 decoding and includes a narrow compatibility-ID migration, but event history grows without retention and migrations still lack a schema framework | Add schema versioning, transactional migrations, retention/compaction, indexes, backup/restore tests, and bounded startup work |
