...
| Selector branch | Multiple stock captures exposed coherent selector-2 and selector-6 assignment/request/carrier branches | Complete selector-5/counter-0 and retained selector-6/counter-3 branches are captured; `.10` proved a constant leading mark insufficient; continuous IQ then identified the prefix as 256 alternating symbols with distinct RF settings; `.11` emitted that two-phase stream but used an over-large offset/deviation and still stopped at 1/6 | Test only the measured `.12` FSCTRL0 `12` / DEVIATN `0x46` correction, preserving the ordinary assignment; capture a fresh new-identity branch before generalizing |
