...
| Selector branch | Multiple stock captures exposed coherent selector-2 and selector-6 assignment/request/carrier branches | Complete selector-5/counter-0 and retained selector-6/counter-3 branches are captured; one local sweep answered both branches without paired continuation; raw IQ exposed a missing 256-symbol selector-5 leading mark; physical probe `.10` added that mark but still stopped at 1/6 and observed fallback to counter `3` | Capture `.10` continuously on air and verify the generated mark polarity, length, and envelope before changing another protocol variable; capture a fresh new-identity branch before generalizing |
